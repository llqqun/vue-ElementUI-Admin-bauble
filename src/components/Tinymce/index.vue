...
export default {
  ...
  methods: {
    ...
    initTinymce() {
      const _this = this
      const eleId = this.inline ? this.eleId : this.tinymceId
      window.tinymce.init({
        selector: `#${eleId}`,
        inline: this.inline,
        ...
        fixed_toolbar_container: `#${eleId}`,
        toolbar: this.toolbar.length > 0 ? this.toolbar : toolbar,
        menubar: this.menubar,
        plugins: plugins,
        content_css: './customTinycss.css',
        content_style: `.panel-body {
            background-color: red;
          }`,
        font_formats:
          '微软雅黑=Microsoft YaHei;宋体=simsun;黑体=SimHei;仿宋=FangSong;楷体=KaiTi;', // 字体
          ...
          input.onchange = function() {
            ...
            switch (meta.filetype) {
              case 'image':
                ...
                post(path, formData)
                  .then((res) => {
                    const fullPath = _this.lookPath
                      ? _this.lookPath + res.data.imgPath
                      : res.data.imgPath
                    callback(fullPath)
                  })
                  .catch(() => {
    ...
    filUploadFun(callback, path, formData) {
      post(path, formData)
        .then((res) => {
          const fullPath = this.lookPath
            ? this.lookPath + res.data.filePath
            : res.data.filePath
          callback(fullPath)
        })
        .catch((error) => {
    ...
    customImageUpload(blobInfo, succFun, failFun) {
      ...
      post(_this.imgUploadPath, formData)
        .then((res) => {
          const fullPath = _this.lookPath
            ? _this.lookPath + res.data.imgPath
            : res.data.imgPath
