...
export default {
  ...
  methods: {
    ...
    getTableData() {
      get('/api/system/monitor/operatelog/page', this.filterForm).then(res => {
    ...
    batchDelete() {
      ...
      this.$confirm('您确定要删除这些数据吗?', '警告', {
        ...
        .then(async() => {
          await post('/api/system/monitor/operatelog/delete/batch', {
            idList: delList
          }).then(() => {
    ...
    handleLook(row) {
      get('\n' +
        '/api/system/monitor/operatelog/detail/' + row.id).then(res => {
